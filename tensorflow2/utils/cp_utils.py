...
def save_metrics_model(epoch, model, losses_accs, path_dict, save_period):
  """학습하는 동안 주어진 period 마다 loss와 acc를 저장하는 함수"""
...
def metric_visualizer(losses_accs, save_path):
  ...
  ax.plot(
          epoch_range, losses_accs['train_losses'],
          color='tab:blue',
          linestyle=':',
          linewidth=2,
          label='Train Loss'
  ...
  ax.plot(
          epoch_range, losses_accs['validation_losses'],
          color='tab:blue',
          label='Validation Loss'
  )
  ax2.plot(
          epoch_range, losses_accs['train_accs'],
          color='tab:orange',
          linestyle=':',
          linewidth=2,
  ...
  ax2.plot(
          epoch_range, losses_accs['validation_accs'],
          color='tab:orange',
  ...
  plt.close() # Ram의 메모리 부족을 방지하기 위해 반드시